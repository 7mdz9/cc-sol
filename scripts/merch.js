...
const cards=await Promise.all(data.map(async item=>{
  ...
  return '<div class="mc rv '+item.revealClass+'"><div class="mc-img">'+svgText+'<div class="mc-layer">Shop Now</div></div><div class="mc-name">'+item.name+'</div><div class="mc-price">'+item.priceAed+' AED</div></div>';
}));
